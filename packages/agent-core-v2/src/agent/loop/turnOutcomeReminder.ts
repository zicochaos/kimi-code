/**
 * `loop` domain (L4) — renders model-visible context for abnormal turn endings.
 *
 * Converts terminal failures and cancellations into compact, XML-safe reminders.
 * Provider failures expose only public categories and status codes; raw provider
 * payloads, request identifiers, and internal infrastructure details stay out of
 * model context.
 */
...
function renderPublicError(error: TurnErrorSummary | undefined): string {
  ...
  switch (error.code) {
    ...
    case 'provider.overloaded':
      return 'The model provider was overloaded.';
