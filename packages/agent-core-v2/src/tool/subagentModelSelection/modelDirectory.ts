...
export const SUBAGENT_MODEL_CHOICES = ['secondary', 'primary'] as const;
export type SubagentModelChoiceToken = (typeof SUBAGENT_MODEL_CHOICES)[number];
...
export function isSubagentModelChoiceToken(value: string): value is SubagentModelChoiceToken {
  return value === 'primary' || value === 'secondary';
}

export function resolvedSubagentModelDirectory(
  models: IModelService,
  catalog: IModelCatalog,
  currentModel?: string,
): SubagentModelDirectoryOptions {
  return {
    models: filterResolvableSubagentModels(models.list(), (alias) => catalog.get(alias)),
    currentModel,
  };
}

/** When exact-alias selection is disabled, restrict the model property to primary/secondary. */
export function parametersWithSubagentModelSelection(
  parameters: Record<string, unknown>,
  exactAliasSelectionEnabled: boolean,
): Record<string, unknown> {
  const properties = parameters['properties'];
  if (typeof properties !== 'object' || properties === null || Array.isArray(properties)) {
    return parameters;
  }
  const model = (properties as Record<string, unknown>)['model'];
  if (typeof model !== 'object' || model === null || Array.isArray(model)) {
    return parameters;
  }
  if (exactAliasSelectionEnabled) {
    // Drop any enum restriction so exact configured aliases are accepted.
    const { enum: _enum, ...rest } = model as Record<string, unknown>;
    return {
      ...parameters,
      properties: {
        ...(properties as Record<string, unknown>),
        model: { ...rest, type: 'string', minLength: 1 },
      },
    };
  }
  // Upstream secondary-model surface: primary/secondary only.
  return {
    ...parameters,
    properties: {
      ...(properties as Record<string, unknown>),
...
): Readonly<Record<string, ModelRecord>> {
...
export function formatSubagentModelDirectory({
  ...
  const lines = entries.map(([alias, configured]) => {
    ...
    if (model.defaultEffort !== undefined && SAFE_THINKING_EFFORTS.has(model.defaultEffort)) {
...
function safeCapabilities(values: readonly string[] | undefined): string[] {
  return (values ?? []).filter((value) => SAFE_CAPABILITIES.has(value)).slice(0, MAX_METADATA_VALUES);
}
