 ...
 * all. The experimental `subagent-model-selection` flag layers on top: the
 * `model` parameter then also accepts any exact configured alias from the
 * whole model directory (advertised via `formatSubagentModelDirectory`,
 * preflighted against `IModelService` + `IModelCatalog`, and bound directly
 * without pool membership). Swarm mode is
...
export class AgentSwarmTool implements IAgentSwarmTool {
  ...
  get parameters(): Record<string, unknown> {
    if (this.exactModelSelectionEnabled()) {
      return parametersWithSubagentModelSelection(AGENT_SWARM_PARAMETERS, true);
    }
