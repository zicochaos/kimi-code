...
function agentSchemaProperties<T = unknown>(exactAliasSelection = false): Record<string, T> {
  const parameters = parametersWithSubagentModelSelection(
    toInputJsonSchema(SubagentToolInputSchema),
    exactAliasSelection,
  ) as { properties: Record<string, T> };
  return parameters.properties;
}

function agentSwarmSchemaProperties<T = unknown>(exactAliasSelection = false): Record<string, T> {
  const parameters = parametersWithSubagentModelSelection(
...
describe('SubagentToolInputSchema', () => {
  ...
  it('widens model to exact aliases when subagent-model-selection is enabled', () => {
    const properties = agentSchemaProperties<{ description?: string; enum?: string[] }>(true);

    expect(properties['model']?.enum).toBeUndefined();
    expect(properties['model']?.description).toContain('exact configured alias');
  });
...
describe('Agent tool execution contract', () => {
  ...
  it('matches the semantic profile permission rule while displaying an alternate model', async () => {
    ...
    const context = createAgentToolContext(
      lifecycle,
      appService(IFlagService, stubFlag((id) => id === 'subagent-model-selection')),
      modelProviderServices(modelCatalogResolving('mock-model', 'alternate')),
      {
        initialConfig: {
          models: {
            alternate: {
  ...
  it('spawns the subagent on an exact configured alias when model selection is enabled', async () => {
  ...
  it('rejects exact aliases when subagent-model-selection is disabled', async () => {
    const lifecycle = createAgentLifecycleStub({ createAgentIds: ['agent-child'] });
    const context = createAgentToolContext(lifecycle, {
      initialConfig: {
        models: {
    ...
    expect(result.isError).toBe(true);
    expect(result.output).toContain('subagent-model-selection');
    expect(lifecycle.create).not.toHaveBeenCalled();
  });
