...
describe('AgentSwarmTool', () => {
  it('applies one subagent_type across templated subagents', async () => {
    ...
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), swarmMode, stubConfig({ defaultModel: 'provider/fast', models: { 'provider/fast': 'fast and cheap' } }), stubFlag(true), stubSwarmCatalog(), stubCallerProfile(), stubModels(), stubModelCatalog());
    const input = {
  ...
  it('does not expose permission rule argument matching', () => {
    const host = mockSwarmHost();
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), stubConfig(), stubFlag(true), stubSwarmCatalog(), stubCallerProfile(), stubModels(), stubModelCatalog());
  ...
  it('ignores model selection for a resume-only swarm', async () => {
    const host = mockSwarmHost();
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), stubConfig(), stubFlag(false), stubSwarmCatalog(), stubCallerProfile(), stubModels(), stubModelCatalog());
    ...
    expect(host.swarmService.run).toHaveBeenCalledWith(
      expect.objectContaining({
        tasks: [expect.objectContaining({ kind: 'resume', resumeAgentId: 'agent-old-1' })],
      }),
    );
  });

  it('validates model selection for item-based swarms', async () => {
    const host = mockSwarmHost();
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), stubConfig(), stubFlag(false), stubSwarmCatalog(), stubCallerProfile({ modelAlias: 'main-model' }), stubModels(), stubModelCatalog());
    ...
    expect(result.isError).toBe(true);
    expect(result.output).toContain('no [secondary_model.models] pool is configured');
    expect(host.swarmService.run).not.toHaveBeenCalled();
  });
  ...
  it('passes the configured subagent timeout to swarm tasks', async () => {
    const host = mockSwarmHost();
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), stubConfig({ timeoutMs: 5_000 }), stubFlag(true), stubSwarmCatalog(), stubCallerProfile(), stubModels(), stubModelCatalog());
  ...
  it('resolves spawn task bindings from the configured model pool default', async () => {
    const host = mockSwarmHost();
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), stubConfig({ defaultModel: 'provider/fast', models: { 'provider/fast': 'fast and cheap', 'provider/smart': 'hard tasks' } }), stubFlag(true), stubSwarmCatalog(), stubCallerProfile({ modelAlias: 'main-model', thinkingLevel: 'high' }), stubModels(), stubModelCatalog());
  ...
  it('lets the tool call opt back into the primary model', async () => {
    const host = mockSwarmHost();
    const tool = new AgentSwarmTool(host.swarmService, makeAgentScopeContext({ agentId: host.callerAgentId, agentScope: '' }), mockSwarmMode(), stubConfig({ defaultModel: 'provider/fast', models: { 'provider/fast': 'fast and cheap' } }), stubFlag(true), stubSwarmCatalog(), stubCallerProfile({ modelAlias: 'main-model', thinkingLevel: 'high' }), stubModels(), stubModelCatalog());
