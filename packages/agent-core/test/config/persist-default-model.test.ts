...
describe('planConfigWrite', () => {
  ...
  it('uses effective flag when disabling persistence with a model patch', () => {
  ...
  it('writes the runtime model when persistence is enabled again', () => {
    const disk = { ...base, persistDefaultModel: false } as KimiConfig;
...
describe('persist_default_model disk behavior', () => {
  ...
  it('model-only write with flag false leaves disk default_model unchanged', () => {
...
`);
  ...
  it('mixed write persists other sections while freezing model and thinking', async () => {
    const path = tempConfig(`
persist_default_model = false
default_model = "disk-model"
[thinking]
