...
| `KIMI_CODE_EXPERIMENTAL_SUBAGENT_MODEL_SELECTION` | Enable exact configured-model aliases on [`Agent` and `AgentSwarm`](../customization/agents.md#how-to-invoke) in addition to `primary`/`secondary` when secondary-model is available | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
