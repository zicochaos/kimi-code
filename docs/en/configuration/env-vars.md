...
| `KIMI_CODE_EXPERIMENTAL_SUBAGENT_MODEL_SELECTION` | Let `Agent` and `AgentSwarm` bind an exact configured model alias via their `model` parameter, on top of the [subagent model pool](./config-files.md#subagent-model-pool) aliases; read by the `agent-core-v2` engine only. The master `KIMI_CODE_EXPERIMENTAL_FLAG=1` also enables it | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
