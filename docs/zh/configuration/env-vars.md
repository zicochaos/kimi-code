...
| `KIMI_CODE_EXPERIMENTAL_SUBAGENT_MODEL_SELECTION` | 为 [`Agent` 和 `AgentSwarm`](../customization/agents.md#调用方式) 启用精确的已配置模型 alias（在已有 `primary`/`secondary` 之上） | 真值：`1`/`true`/`yes`/`on`；假值：`0`/`false`/`no`/`off` |
