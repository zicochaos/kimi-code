...
| `KIMI_CODE_EXPERIMENTAL_SUBAGENT_MODEL_SELECTION` | 允许 `Agent` 和 `AgentSwarm` 通过 `model` 参数绑定已配置模型的精确别名，在 [subagent 模型池](./config-files.md#subagent-模型池)别名之上；仅由 `agent-core-v2` 引擎读取。master `KIMI_CODE_EXPERIMENTAL_FLAG=1` 也会启用本功能 | 真值：`1`/`true`/`yes`/`on`；假值：`0`/`false`/`no`/`off` |
