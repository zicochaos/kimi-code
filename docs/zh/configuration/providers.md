...
- ↑/↓ 移动光标，←/→ 翻页
- `d` 键删除当前供应商（有 `[y/N]` 确认）
- 在单个供应商行按 Enter 编辑该供应商，或在 `[ Add New Platform ]` 行按 Enter 添加新供应商

添加时有三条路径：

- **OpenAI-compatible provider**：输入供应商 ID、base URL 和 API 密钥；Kimi Code 会拉取 `${baseUrl}/models`，自动创建模型 alias，然后打开 `/model` 供你选择
