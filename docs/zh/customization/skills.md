...
名称匹配不区分大小写。被禁用的 Skill 会从模型列表中移除，被 `Skill` 工具拒绝，不出现在斜杠菜单中，也无法由用户激活。被禁用的 Skill 也不会由插件的 `sessionStart` hook 注入——无论是在会话启动时还是 reload 之后。磁盘上的文件会保留，供其他工具使用。修改 `config.toml` 后执行 `/reload`；当前会话会立即刷新，无需重启。
